...
template <typename T> struct data_of_type : utils::noncopyable {
    ...
    size_t values_size;
    size_t value_index;
    data_of_type(T const *values_, size_t values_size_) : values(values_), values_size(values_size_), value_index(0) {}
...
#define FEM_LOC(V)                              \
    ...
    FEM_LOC(double)
#undef FEM_LOC
    template <typename OtherT, size_t Ndims> data_of_type &operator,(arr_ref<OtherT, Ndims> &val) {
...
struct data_of_type_str : utils::noncopyable {
    ...
    size_t values_size;
    size_t value_index;
    data_of_type_str(char const **values_, size_t values_size_) : values(values_), values_size(values_size_), value_index(0) {}
    ...
    template <int StrLen, size_t Ndims> data_of_type_str &operator,(arr_ref<str<StrLen>, Ndims> &val) {
        size_t n = val.size_1d();
    ...
    template <size_t Ndims> data_of_type_str &operator,(str_arr_ref<Ndims> &val) {
        size_t n = val.size_1d();
        for (size_t i = 0; i < n; i++)
            (*this), val[i];
        return *this;
