...
def tk_double_precision(ssl, i_code, value): return token(
    4, ssl, i_code, value)
...
class ssl_iterator(object):
        ...
        def return_none():
            if (not optional):
                if (stop == 0):
                    ...
                    ssl.raise_syntax_error(i=stop-1)
            O.i = None
            return None
        if (len(O.buffer) != 0):
    ...
    def __after_dot(O, i_fld, i_dot):
        ...
        if (j < 0):
            ssl.raise_syntax_error(i=i_dot+1)
        O.i = j
        if (code.find("d", i_dot+1, O.i) < 0):
            tk_type = tk_real
        else:
            tk_type = tk_double_precision
...
class implied_do_info(object):
    ...
    def __init__(O, tokens):
        ...
        def get_first_tokens(i):
            ...
            if (len(tv) < 3):
                return False
            if (not tv[0].is_identifier()):
                return False
            if (not tv[1].is_op_with(value="=")):
                return False
            O.dlist_size = len(tokens) + i
            O.id_tok = tv[0]
            O.fls_tokens = [
                tk_seq(ssl=tv[2].ssl, i_code=tv[2].i_code, value=tv[2:])]
            return True
