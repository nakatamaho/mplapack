...
cat hand/*hpp *hpp ~/mplapack/misc/special.hpp | grep -v abs1 | grep -v abs2 | grep -v abssq | grep -v arr_c | grep -v common | grep -v UNHANDLED_function | grep -v arr_ref | grep -v  mplapack_Rlaruv_initialize | grep -v random_mplapack_gmp |sort  |uniq > header_all
...
for mplib in $MPLIBS; do
    ...
    cat ~/mplapack/fable/mplapack_matgen_${mplib}.h.in mplapack_matgen_${mplib}.h > ~/mplapack/include/mplapack_matgen_${mplib}.h
