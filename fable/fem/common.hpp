...
struct cmn_sve {
  ...
  public:
    ...
    template <typename T> void construct() { content = new holder<T>(new T); }
    template <typename T, typename D> void construct(D const &dynamic_parameters) { content = new holder<T>(new T(dynamic_parameters)); }
    template <typename T> T &get() { return *(static_cast<holder<T> *>(content)->ptr); }
};
template <typename T> void no_operation_to_avoid_unused_variable_warning(const T &) {}
} // namespace fem
