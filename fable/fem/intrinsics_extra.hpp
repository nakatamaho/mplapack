...
inline void date(str_ref result) {
    ...
    str<10> buf;
    std::snprintf(buf.elems, sizeof(buf.elems), "%02d-%s-%02d", tm->tm_mday, months[tm->tm_mon], tm->tm_year % 100);
    result = buf;
}
inline void time(str_ref result) {
    ...
    std::tm const *tm(std::localtime(&now));
    str<10> buf;
    std::snprintf(buf.elems, sizeof(buf.elems), "%02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
    result = buf;
}
...
inline void cpu_time(float &result) { result = static_cast<float>(user_plus_system_time()); }
inline void cpu_time(double &result) { result = user_plus_system_time(); }
inline int system(str_cref command) { return std::system(std::string(command).c_str()); }
} // namespace fem
#endif // GUARD
