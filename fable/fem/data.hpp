...
struct datum {
    ...
    template <typename T> datum(T const &value) : content(new holder<std::string>(std::string(value))), repeats(1) {}
    ...
    void throw_type_mismatch(char const *target_type) const { throw std::runtime_error(std::string("DATA type mismatch: target type ") + target_type + " vs. " + content->type().name() + " value"); }
};
...
struct data_buffer {
    ...
    template <typename T> data_buffer &operator,(T const &val) {
...
struct data_values {
    ...
    void report_types() const {
        ...
        for (size_t i = 0; i != n; i++) {
            ...
            std::cout << "data_buffer[" << i << "]: " << obj.content->type().name() << std::endl;
        }
    }
    ...
    template <typename T, size_t Ndims> data_values &operator,(arr_ref<T, Ndims> &val) {
    ...
    template <size_t Ndims> data_values &operator,(str_arr_ref<Ndims> &val) {
        size_t n = val.size_1d();
        for (size_t i = 0; i < n; i++)
            (*this), val[i];
        return *this;
...
struct values_type {
    template <typename T> data_buffer operator,(T const &val) const { return data_buffer(), val; }
};
static const values_type values = values_type();
inline data_values data(data_buffer const &values) { return data_values(values); }
} // namespace fem
#endif // GUARD
