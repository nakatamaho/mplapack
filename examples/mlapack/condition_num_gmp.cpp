...
void getAinv(mpackint n, mpackint lda, mpf_class * A)
{
    mpackint info;
    mpackint lwork;
...
mpf_class get_estimated_condition_num(const char *norm, mpackint n, mpackint lda, mpf_class * A)
{
    mpf_class anorm, cond, rcond;
    mpf_class *work = new mpf_class[std::max((mpackint) 1, n * 4)];
...
mpf_class get_exact_condition_num(const char *norm, mpackint n, mpackint lda, mpf_class * A)
{
    mpf_class *Ainv = new mpf_class[n * n];
    mpf_class *work = new mpf_class[std::max((mpackint) 1, n)];
