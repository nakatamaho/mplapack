...
int main(){ mplapackint n=12,lda=n,ldb=n,info; mpf_class *a=new mpf_class[n*n]; mpf_class *aorg=new mpf_class[n*n]; mpf_class *b=new mpf_class[n]; mpf_class *xexact=new mpf_class[n]; mplapackint *ipiv=new mplapackint[n]; for(mplapackint j=0;j<n;j++) for(mplapackint i=0;i<n;i++){ a[i+j*lda]=mpf_class(1.0)/mpf_class(i+j+1); aorg[i+j*lda]=a[i+j*lda]; } for(mplapackint i=0;i<n;i++) xexact[i]=(i%2==0)?mpf_class(1.0):mpf_class(-1.0); for(mplapackint i=0;i<n;i++){ b[i]=0; for(mplapackint k=0;k<n;k++) b[i]=b[i]+aorg[i+k*lda]*xexact[k]; } printf("Hilbert n = %ld\n",(long)n); Rgesv(n,(mplapackint)1,a,lda,ipiv,b,ldb,info); printf("max |x-x_exact| = "); printnum(max_solution_error(n,b,xexact)); printf("\n"); delete[] ipiv; delete[] xexact; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
