...
int main(){ mplapackint n=20,lda=n,ldv=1,info,lwork=-1; dd_real *a=new dd_real[n*n]; for(mplapackint j=0;j<n;j++) for(mplapackint i=0;i<n;i++) a[i+j*lda]=(i<=j)?dd_real(n-j):dd_real(n-i); dd_real *wr=new dd_real[n]; dd_real *wi=new dd_real[n]; dd_real *vl=new dd_real[1]; dd_real *vr=new dd_real[1]; dd_real wk; Rgeev("N","N",n,a,lda,wr,wi,vl,ldv,vr,ldv,&wk,lwork,info); lwork=castINTEGER_dd(wk); dd_real *work=new dd_real[lwork]; Rgeev("N","N",n,a,lda,wr,wi,vl,ldv,vr,ldv,work,lwork,info); sort_real(n,wr); printf("Frank eigenvalues = "); printvec(wr,n); printf("\n"); delete[] work; delete[] vr; delete[] vl; delete[] wi; delete[] wr; delete[] a; return info!=0?1:0; }
