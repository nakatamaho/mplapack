...
int main(){ mplapackint n=2,nrhs=1,lda=n,ldb=n,info,lwork=-1; std::complex<double> *a=new std::complex<double>[n*n]; std::complex<double> *aorg=new std::complex<double>[n*n]; std::complex<double> *b=new std::complex<double>[n]; std::complex<double> *borg=new std::complex<double>[n]; std::complex<double> *xexact=new std::complex<double>[n]; mplapackint *ipiv=new mplapackint[n]; a[0]=std::complex<double>(2.0,0.0); a[1]=std::complex<double>(1.0,1.0); a[2]=std::complex<double>(1.0,1.0); a[3]=std::complex<double>(-3.0,0.0); xexact[0]=std::complex<double>(1.0,-1.0); xexact[1]=std::complex<double>(2.0,1.0); for(mplapackint i=0;i<n*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<n;i++){ b[i]=std::complex<double>(0.0,0.0); for(mplapackint k=0;k<n;k++) b[i]=b[i]+aorg[i+k*lda]*xexact[k]; borg[i]=b[i]; } std::complex<double> wk; Csysv("U",n,nrhs,a,lda,ipiv,b,ldb,&wk,lwork,info); lwork=castINTEGER_double(wk.real()); std::complex<double> *work=new std::complex<double>[lwork]; Csysv("U",n,nrhs,a,lda,ipiv,b,ldb,work,lwork,info); printf("A = "); printmat(n,n,aorg,lda); printf("\n"); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(n,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] ipiv; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
