...
int main(){ mplapackint n=2,nrhs=1,lda=n,ldb=n,info,lwork=-1; qd_complex *a=new qd_complex[n*n]; qd_complex *aorg=new qd_complex[n*n]; qd_complex *b=new qd_complex[n]; qd_complex *borg=new qd_complex[n]; qd_complex *xexact=new qd_complex[n]; mplapackint *ipiv=new mplapackint[n]; a[0]=qd_complex(2.0,0.0); a[1]=qd_complex(1.0,1.0); a[2]=qd_complex(1.0,1.0); a[3]=qd_complex(-3.0,0.0); xexact[0]=qd_complex(1.0,-1.0); xexact[1]=qd_complex(2.0,1.0); for(mplapackint i=0;i<n*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<n;i++){ b[i]=qd_complex(0.0,0.0); for(mplapackint k=0;k<n;k++) b[i]=b[i]+aorg[i+k*lda]*xexact[k]; borg[i]=b[i]; } qd_complex wk; Csysv("U",n,nrhs,a,lda,ipiv,b,ldb,&wk,lwork,info); lwork=castINTEGER_qd(wk.real()); qd_complex *work=new qd_complex[lwork]; Csysv("U",n,nrhs,a,lda,ipiv,b,ldb,work,lwork,info); printf("A = "); printmat(n,n,aorg,lda); printf("\n"); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(n,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] ipiv; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
