...
qd_real one_norm(mplapackint n, qd_real *a, mplapackint lda) {
    qd_real anorm = 0.0;
    for (mplapackint j = 0; j < n; j++) { qd_real s = 0.0; for (mplapackint i = 0; i < n; i++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
qd_real inf_norm(mplapackint n, qd_real *a, mplapackint lda) {
    qd_real anorm = 0.0;
    for (mplapackint i = 0; i < n; i++) { qd_real s = 0.0; for (mplapackint j = 0; j < n; j++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
int main() {
    ...
    a[0 + 0 * lda] = 1; a[1 + 1 * lda] = qd_real(1.0e-3); a[2 + 2 * lda] = qd_real(1.0e-6);
