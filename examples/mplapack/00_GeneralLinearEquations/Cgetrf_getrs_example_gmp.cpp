...
int main() {
    ...
    mpfc_class *b = new mpfc_class[nrhs * ldb];
    mpfc_class *borg = new mpfc_class[nrhs * ldb];
    mpfc_class *xexact = new mpfc_class[nrhs * n];
    mplapackint *ipiv = new mplapackint[n];

    a[0 + 0 * lda] = mpfc_class(2.0, 0.0); a[0 + 1 * lda] = mpfc_class(1.0, 1.0);
    a[1 + 0 * lda] = mpfc_class(1.0, -1.0); a[1 + 1 * lda] = mpfc_class(3.0, 0.0);
    xexact[0] = mpfc_class(1.0, 0.0); xexact[1] = mpfc_class(0.0, 1.0);
    xexact[0 + 1 * n] = mpfc_class(-1.0, 2.0); xexact[1 + 1 * n] = mpfc_class(2.0, -1.0);
