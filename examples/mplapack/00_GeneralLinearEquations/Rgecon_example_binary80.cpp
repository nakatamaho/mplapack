...
int main() {
    ...
    mplapack_binary80_t *lu = new mplapack_binary80_t[n * n];
    ...
    a[0 + 0 * lda] = 1.0; a[1 + 1 * lda] = mplapack_binary80_t(1.0e-3); a[2 + 2 * lda] = mplapack_binary80_t(1.0e-6);
    for (mplapackint i = 0; i < n * n; i++) lu[i] = a[i];
    Rgetrf(n, n, lu, lda, ipiv, info);
    mplapack_binary80_t rcond1 = 0.0, rcondi = 0.0;
