...
int main() {
    ...
    a[0 + 0 * lda] = std::complex<double>(2.0, 0.0); a[0 + 1 * lda] = std::complex<double>(1.0, 1.0);
    a[1 + 0 * lda] = std::complex<double>(1.0, -1.0); a[1 + 1 * lda] = std::complex<double>(3.0, 0.0);
    xexact[0] = std::complex<double>(1.0, 0.0); xexact[1] = std::complex<double>(0.0, 1.0);
    xexact[0 + 1 * n] = std::complex<double>(-1.0, 2.0); xexact[1 + 1 * n] = std::complex<double>(2.0, -1.0);
    for (mplapackint i = 0; i < n * n; i++) aorg[i] = a[i];
    for (mplapackint j = 0; j < nrhs; j++) for (mplapackint i = 0; i < n; i++) {
        b[i + j * ldb] = std::complex<double>(0.0, 0.0);
