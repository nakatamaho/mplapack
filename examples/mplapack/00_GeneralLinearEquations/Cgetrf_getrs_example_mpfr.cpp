...
int main() {
    ...
    mpc_class *b = new mpc_class[nrhs * ldb];
    mpc_class *borg = new mpc_class[nrhs * ldb];
    mpc_class *xexact = new mpc_class[nrhs * n];
    mplapackint *ipiv = new mplapackint[n];

    a[0 + 0 * lda] = mpc_class(2.0, 0.0); a[0 + 1 * lda] = mpc_class(1.0, 1.0);
    a[1 + 0 * lda] = mpc_class(1.0, -1.0); a[1 + 1 * lda] = mpc_class(3.0, 0.0);
    xexact[0] = mpc_class(1.0, 0.0); xexact[1] = mpc_class(0.0, 1.0);
    xexact[0 + 1 * n] = mpc_class(-1.0, 2.0); xexact[1 + 1 * n] = mpc_class(2.0, -1.0);
