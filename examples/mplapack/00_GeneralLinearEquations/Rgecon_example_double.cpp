...
double one_norm(mplapackint n, double *a, mplapackint lda) {
    double anorm = 0.0;
    for (mplapackint j = 0; j < n; j++) { double s = 0.0; for (mplapackint i = 0; i < n; i++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
double inf_norm(mplapackint n, double *a, mplapackint lda) {
    double anorm = 0.0;
    for (mplapackint i = 0; i < n; i++) { double s = 0.0; for (mplapackint j = 0; j < n; j++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
int main() {
    ...
    a[0 + 0 * lda] = 1; a[1 + 1 * lda] = double(1.0e-3); a[2 + 2 * lda] = double(1.0e-6);
