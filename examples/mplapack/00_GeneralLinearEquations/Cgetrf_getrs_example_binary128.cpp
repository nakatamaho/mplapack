...
int main() {
    ...
    std::complex<mplapack_binary128_t> *b = new std::complex<mplapack_binary128_t>[nrhs * ldb];
    std::complex<mplapack_binary128_t> *borg = new std::complex<mplapack_binary128_t>[nrhs * ldb];
    std::complex<mplapack_binary128_t> *xexact = new std::complex<mplapack_binary128_t>[nrhs * n];
    mplapackint *ipiv = new mplapackint[n];

    a[0 + 0 * lda] = std::complex<mplapack_binary128_t>(2.0, 0.0); a[0 + 1 * lda] = std::complex<mplapack_binary128_t>(1.0, 1.0);
    a[1 + 0 * lda] = std::complex<mplapack_binary128_t>(1.0, -1.0); a[1 + 1 * lda] = std::complex<mplapack_binary128_t>(3.0, 0.0);
    xexact[0] = std::complex<mplapack_binary128_t>(1.0, 0.0); xexact[1] = std::complex<mplapack_binary128_t>(0.0, 1.0);
    xexact[0 + 1 * n] = std::complex<mplapack_binary128_t>(-1.0, 2.0); xexact[1 + 1 * n] = std::complex<mplapack_binary128_t>(2.0, -1.0);
    for (mplapackint i = 0; i < n * n; i++) aorg[i] = a[i];
    for (mplapackint j = 0; j < nrhs; j++) for (mplapackint i = 0; i < n; i++) {
        b[i + j * ldb] = std::complex<mplapack_binary128_t>(0.0, 0.0);
