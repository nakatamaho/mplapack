...
mpreal one_norm(mplapackint n, mpreal *a, mplapackint lda) {
    mpreal anorm = 0.0;
    for (mplapackint j = 0; j < n; j++) { mpreal s = 0.0; for (mplapackint i = 0; i < n; i++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
mpreal inf_norm(mplapackint n, mpreal *a, mplapackint lda) {
    mpreal anorm = 0.0;
    for (mplapackint i = 0; i < n; i++) { mpreal s = 0.0; for (mplapackint j = 0; j < n; j++) s = s + abs(a[i + j * lda]); if (anorm < s) anorm = s; }
    return anorm;
}
int main() {
    mplapackint n = 3, lda = n, info;
    mpreal *a = new mpreal[n * n];
    mpreal *lu = new mpreal[n * n];
    mpreal *work = new mpreal[4 * n];
    mplapackint *iwork = new mplapackint[n];
    mplapackint *ipiv = new mplapackint[n];
    for (mplapackint i = 0; i < n * n; i++) a[i] = 0;
    a[0 + 0 * lda] = 1; a[1 + 1 * lda] = mpreal(1.0e-3); a[2 + 2 * lda] = mpreal(1.0e-6);
    for (mplapackint i = 0; i < n * n; i++) lu[i] = a[i];
    Rgetrf(n, n, lu, lda, ipiv, info);
    mpreal rcond1 = 0.0, rcondi = 0.0;
    ...
    printf("true cond_1 = "); printnum(mpreal(1.0e6)); printf("\n");
