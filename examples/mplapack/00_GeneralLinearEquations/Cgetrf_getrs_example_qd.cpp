...
int main() {
    ...
    qd_complex *b = new qd_complex[nrhs * ldb];
    qd_complex *borg = new qd_complex[nrhs * ldb];
    qd_complex *xexact = new qd_complex[nrhs * n];
    mplapackint *ipiv = new mplapackint[n];

    a[0 + 0 * lda] = qd_complex(2.0, 0.0); a[0 + 1 * lda] = qd_complex(1.0, 1.0);
    a[1 + 0 * lda] = qd_complex(1.0, -1.0); a[1 + 1 * lda] = qd_complex(3.0, 0.0);
    xexact[0] = qd_complex(1.0, 0.0); xexact[1] = qd_complex(0.0, 1.0);
    xexact[0 + 1 * n] = qd_complex(-1.0, 2.0); xexact[1 + 1 * n] = qd_complex(2.0, -1.0);
    for (mplapackint i = 0; i < n * n; i++) aorg[i] = a[i];
    for (mplapackint j = 0; j < nrhs; j++) for (mplapackint i = 0; i < n; i++) {
        b[i + j * ldb] = qd_complex(0.0, 0.0);
