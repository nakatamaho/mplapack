...
int main(){ mplapackint m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1,rank; mplapack_binary80_t *a=new mplapack_binary80_t[lda*n]; mplapack_binary80_t *aorg=new mplapack_binary80_t[lda*n]; mplapack_binary80_t *b=new mplapack_binary80_t[ldb]; mplapack_binary80_t *borg=new mplapack_binary80_t[ldb]; mplapack_binary80_t *xexact=new mplapack_binary80_t[n]; mplapack_binary80_t *s=new mplapack_binary80_t[n]; set_problem(m,n,a,lda,b,ldb,xexact); for(mplapackint i=0;i<lda*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<ldb;i++) borg[i]=b[i]; mplapack_binary80_t wk; Rgelss(m,n,nrhs,a,lda,b,ldb,s,mplapack_binary80_t(-1.0),rank,&wk,lwork,info); lwork=castINTEGER_binary80(wk); mplapack_binary80_t *work=new mplapack_binary80_t[lwork]; Rgelss(m,n,nrhs,a,lda,b,ldb,s,mplapack_binary80_t(-1.0),rank,work,lwork,info); printf("singular values = "); printvec(s,n); printf("\n"); printf("rank = %ld\n", (long)rank); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] s; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
