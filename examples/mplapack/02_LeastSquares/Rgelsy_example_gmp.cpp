...
int main() {
    ...
    mpf_class *aorg = new mpf_class[lda * n];
    mpf_class *b = new mpf_class[ldb];
    mpf_class *borg = new mpf_class[ldb];
    ...
    for (mplapackint i = 0; i < ldb; i++)
        borg[i] = b[i];
    mpf_class wk;
    Rgelsy(m, n, nrhs, a, lda, b, ldb, jpvt, mpf_class(-1.0), rank, &wk, lwork, info);
    lwork = castINTEGER_gmp(wk);
    mpf_class *work = new mpf_class[lwork];
    Rgelsy(m, n, nrhs, a, lda, b, ldb, jpvt, mpf_class(-1.0), rank, work, lwork, info);
