...
int main(){ INTEGER m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1,rank; REAL *a=new REAL[lda*n]; REAL *aorg=new REAL[lda*n]; REAL *b=new REAL[ldb]; REAL *borg=new REAL[ldb]; REAL *xexact=new REAL[n]; REAL *s=new REAL[n]; set_problem(m,n,a,lda,b,ldb,xexact); for(INTEGER i=0;i<lda*n;i++) aorg[i]=a[i]; for(INTEGER i=0;i<ldb;i++) borg[i]=b[i]; REAL wk; Rgelss(m,n,nrhs,a,lda,b,ldb,s,REAL(-1.0),rank,&wk,lwork,info); lwork=castInTEGER(wk); REAL *work=new REAL[lwork]; Rgelss(m,n,nrhs,a,lda,b,ldb,s,REAL(-1.0),rank,work,lwork,info); printf("singular values = "); printvec(s,n); printf("\n"); printf("rank = %ld\n", (long)rank); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] s; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
