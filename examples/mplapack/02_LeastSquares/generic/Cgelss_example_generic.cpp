...
void set_problem(INTEGER m, INTEGER n, COMPLEX *a, INTEGER lda, COMPLEX *b, INTEGER ldb, COMPLEX *xexact) {
    for (INTEGER i = 0; i < m; i++) {
        a[i + 0 * lda] = COMPLEX(1.0, 0.0);
        a[i + 1 * lda] = COMPLEX(i, 1.0);
    }
    xexact[0] = COMPLEX(1.0, -1.0);
    xexact[1] = COMPLEX(2.0, 1.0);
    for (INTEGER i = 0; i < m; i++)
...
int main() {
    ...
    COMPLEX *a = new COMPLEX[lda * n];
    COMPLEX *aorg = new COMPLEX[lda * n];
    COMPLEX *b = new COMPLEX[ldb];
    COMPLEX *borg = new COMPLEX[ldb];
    COMPLEX *xexact = new COMPLEX[n];
    REAL *s = new REAL[n];
    REAL *rwork = new REAL[5 * n];
    ...
    for (INTEGER i = 0; i < ldb; i++)
        borg[i] = b[i];
    COMPLEX wk;
    Cgelss(m, n, nrhs, a, lda, b, ldb, s, REAL(-1.0), rank, &wk, lwork, rwork, info);
    lwork = castInTEGER(wk.real());
    COMPLEX *work = new COMPLEX[lwork];
    Cgelss(m, n, nrhs, a, lda, b, ldb, s, REAL(-1.0), rank, work, lwork, rwork, info);
