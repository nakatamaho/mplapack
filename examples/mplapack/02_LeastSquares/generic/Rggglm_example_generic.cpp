...
int main() {
    INTEGER n = 3, m = 2, p = 3, lda = n, ldb = n, info, lwork = -1;
    REAL *a = new REAL[lda * m];
    REAL *b = new REAL[ldb * p];
    REAL *d = new REAL[n];
    REAL *x = new REAL[m];
    REAL *y = new REAL[p];
    REAL *xexact = new REAL[m];
    REAL *yexact = new REAL[p];
    for (INTEGER i = 0; i < lda * m; i++)
        a[i] = 0;
    a[0] = 1;
    a[1] = 0;
    a[2] = 1;
