...
int main() {
    ...
    double *aorg = new double[lda * n];
    double *b = new double[ldb];
    double *borg = new double[ldb];
    ...
    for (mplapackint i = 0; i < ldb; i++)
        borg[i] = b[i];
    double wk;
    Rgelsy(m, n, nrhs, a, lda, b, ldb, jpvt, double(-1.0), rank, &wk, lwork, info);
    lwork = castINTEGER_double(wk);
    double *work = new double[lwork];
    Rgelsy(m, n, nrhs, a, lda, b, ldb, jpvt, double(-1.0), rank, work, lwork, info);
