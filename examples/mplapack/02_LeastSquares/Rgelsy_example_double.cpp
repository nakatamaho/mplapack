...
int main(){ mplapackint m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1,rank; double *a=new double[lda*n]; double *aorg=new double[lda*n]; double *b=new double[ldb]; double *borg=new double[ldb]; double *xexact=new double[n]; mplapackint *jpvt=new mplapackint[n]; for(mplapackint i=0;i<n;i++) jpvt[i]=0; set_problem(m,n,a,lda,b,ldb,xexact); for(mplapackint i=0;i<lda*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<ldb;i++) borg[i]=b[i]; double wk; Rgelsy(m,n,nrhs,a,lda,b,ldb,jpvt,double(-1.0),rank,&wk,lwork,info); lwork=castINTEGER_double(wk); double *work=new double[lwork]; Rgelsy(m,n,nrhs,a,lda,b,ldb,jpvt,double(-1.0),rank,work,lwork,info); printf("A = "); printmat(m,n,aorg,lda); printf("\n"); printf("x = "); printvec(b,n); printf("\n"); printf("rank = %ld\n", (long)rank); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] jpvt; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
