...
int main(){ mplapackint m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1,rank; mpc_class *a=new mpc_class[lda*n]; mpc_class *aorg=new mpc_class[lda*n]; mpc_class *b=new mpc_class[ldb]; mpc_class *borg=new mpc_class[ldb]; mpc_class *xexact=new mpc_class[n]; mpf_class *s=new mpf_class[n]; mpf_class *rwork=new mpf_class[5*n]; set_problem(m,n,a,lda,b,ldb,xexact); for(mplapackint i=0;i<lda*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<ldb;i++) borg[i]=b[i]; mpc_class wk; Cgelss(m,n,nrhs,a,lda,b,ldb,s,mpf_class(-1.0),rank,&wk,lwork,rwork,info); lwork=castINTEGER_gmp(wk.real()); mpc_class *work=new mpc_class[lwork]; Cgelss(m,n,nrhs,a,lda,b,ldb,s,mpf_class(-1.0),rank,work,lwork,rwork,info); printf("singular values = "); printvec(s,n); printf("\n"); printf("rank = %ld\n", (long)rank); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] rwork; delete[] s; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
