...
int main(){ mplapackint n=3,m=2,p=3,lda=n,ldb=n,info,lwork=-1; qd_real *a=new qd_real[lda*m]; qd_real *b=new qd_real[ldb*p]; qd_real *d=new qd_real[n]; qd_real *x=new qd_real[m]; qd_real *y=new qd_real[p]; qd_real *xexact=new qd_real[m]; qd_real *yexact=new qd_real[p]; for(mplapackint i=0;i<lda*m;i++) a[i]=0; a[0]=1; a[1]=0; a[2]=1; a[0+lda]=0; a[1+lda]=1; a[2+lda]=1; for(mplapackint i=0;i<ldb*p;i++) b[i]=0; for(mplapackint i=0;i<n;i++) b[i+i*ldb]=1; xexact[0]=1; xexact[1]=2; yexact[0]=qd_real(0.5); yexact[1]=qd_real(-0.5); yexact[2]=1; for(mplapackint i=0;i<n;i++) d[i]=a[i]*xexact[0]+a[i+lda]*xexact[1]+yexact[i]; qd_real wk; Rggglm(n,m,p,a,lda,b,ldb,d,x,y,&wk,lwork,info); lwork=castINTEGER_qd(wk); qd_real *work=new qd_real[lwork]; Rggglm(n,m,p,a,lda,b,ldb,d,x,y,work,lwork,info); printf("x = "); printvec(x,m); printf("\n"); printf("y = "); printvec(y,p); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(m,(mplapackint)1,x,m,xexact,m)); printf("\n"); printf("max |y-y_exact| = "); printnum(max_solution_error(p,(mplapackint)1,y,p,yexact,p)); printf("\n"); delete[] work; delete[] yexact; delete[] xexact; delete[] y; delete[] x; delete[] d; delete[] b; delete[] a; return info!=0?1:0; }
