...
int main(){ mplapackint m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1; std::complex<mplapack_binary80_t> *a=new std::complex<mplapack_binary80_t>[lda*n]; std::complex<mplapack_binary80_t> *aorg=new std::complex<mplapack_binary80_t>[lda*n]; std::complex<mplapack_binary80_t> *b=new std::complex<mplapack_binary80_t>[ldb]; std::complex<mplapack_binary80_t> *borg=new std::complex<mplapack_binary80_t>[ldb]; std::complex<mplapack_binary80_t> *xexact=new std::complex<mplapack_binary80_t>[n]; set_problem(m,n,a,lda,b,ldb,xexact); for(mplapackint i=0;i<lda*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<ldb;i++) borg[i]=b[i]; std::complex<mplapack_binary80_t> wk; Cgels("N",m,n,nrhs,a,lda,b,ldb,&wk,lwork,info); lwork=castINTEGER_binary80(wk.real()); std::complex<mplapack_binary80_t> *work=new std::complex<mplapack_binary80_t>[lwork]; Cgels("N",m,n,nrhs,a,lda,b,ldb,work,lwork,info); printf("A = "); printmat(m,n,aorg,lda); printf("\n"); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
