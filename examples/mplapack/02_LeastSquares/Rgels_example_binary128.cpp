...
int main(){ mplapackint m=4,n=2,nrhs=1,lda=m,ldb=m,info,lwork=-1; mplapack_binary128_t *a=new mplapack_binary128_t[lda*n]; mplapack_binary128_t *aorg=new mplapack_binary128_t[lda*n]; mplapack_binary128_t *b=new mplapack_binary128_t[ldb]; mplapack_binary128_t *borg=new mplapack_binary128_t[ldb]; mplapack_binary128_t *xexact=new mplapack_binary128_t[n]; set_problem(m,n,a,lda,b,ldb,xexact); for(mplapackint i=0;i<lda*n;i++) aorg[i]=a[i]; for(mplapackint i=0;i<ldb;i++) borg[i]=b[i]; mplapack_binary128_t wk; Rgels("N",m,n,nrhs,a,lda,b,ldb,&wk,lwork,info); lwork=castINTEGER_binary128(wk); mplapack_binary128_t *work=new mplapack_binary128_t[lwork]; Rgels("N",m,n,nrhs,a,lda,b,ldb,work,lwork,info); printf("A = "); printmat(m,n,aorg,lda); printf("\n"); printf("x = "); printvec(b,n); printf("\n"); printf("max |x-x_exact| = "); printnum(max_solution_error(n,nrhs,b,ldb,xexact,n)); printf("\n"); printf("max residual = "); printnum(max_residual(m,n,nrhs,aorg,lda,b,ldb,borg,ldb)); printf("\n"); delete[] work; delete[] xexact; delete[] borg; delete[] b; delete[] aorg; delete[] a; return info!=0?1:0; }
