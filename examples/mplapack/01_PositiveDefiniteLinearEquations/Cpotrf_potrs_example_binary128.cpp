...
int main() {
    ...
    std::complex<mplapack_binary128_t> *a = new std::complex<mplapack_binary128_t>[n*n]; std::complex<mplapack_binary128_t> *aorg = new std::complex<mplapack_binary128_t>[n*n]; std::complex<mplapack_binary128_t> *b = new std::complex<mplapack_binary128_t>[n*nrhs]; std::complex<mplapack_binary128_t> *borg = new std::complex<mplapack_binary128_t>[n*nrhs]; std::complex<mplapack_binary128_t> *xexact = new std::complex<mplapack_binary128_t>[n*nrhs];
    a[0]=std::complex<mplapack_binary128_t>(5.0,0.0); a[1]=std::complex<mplapack_binary128_t>(1.0,-1.0); a[2]=std::complex<mplapack_binary128_t>(1.0,1.0); a[3]=std::complex<mplapack_binary128_t>(4.0,0.0);
    xexact[0]=std::complex<mplapack_binary128_t>(1.0,1.0); xexact[1]=std::complex<mplapack_binary128_t>(2.0,-1.0); xexact[0+n]=std::complex<mplapack_binary128_t>(-1.0,0.0); xexact[1+n]=std::complex<mplapack_binary128_t>(0.0,2.0);
    for(mplapackint i=0;i<n*n;i++) aorg[i]=a[i];
    for(mplapackint j=0;j<nrhs;j++) for(mplapackint i=0;i<n;i++){ b[i+j*ldb]=std::complex<mplapack_binary128_t>(0.0,0.0); for(mplapackint k=0;k<n;k++) b[i+j*ldb]=b[i+j*ldb]+aorg[i+k*lda]*xexact[k+j*n]; borg[i+j*ldb]=b[i+j*ldb]; }
