...
double one_norm(mplapackint n, double *a, mplapackint lda) { double anorm=0.0; for(mplapackint j=0;j<n;j++){ double s=0.0; for(mplapackint i=0;i<n;i++) s=s+abs(a[i+j*lda]); if(anorm<s) anorm=s; } return anorm; }
int main() {
    mplapackint n = 2, lda = n, info;
    double *a = new double[n*n]; double *aorg = new double[n*n]; double *work = new double[3*n]; mplapackint *iwork = new mplapackint[n]; double rcond=0.0;
