...
int main() {
    ...
    COMPLEX *a = new COMPLEX[n*n]; COMPLEX *aorg = new COMPLEX[n*n]; COMPLEX *b = new COMPLEX[n]; COMPLEX *borg = new COMPLEX[n]; COMPLEX *xexact = new COMPLEX[n];
    ...
    for(INTEGER i=0;i<n;i++){ b[i]=COMPLEX(0.0,0.0); for(INTEGER k=0;k<n;k++) b[i]=b[i]+aorg[i+k*lda]*xexact[k]; borg[i]=b[i]; }
