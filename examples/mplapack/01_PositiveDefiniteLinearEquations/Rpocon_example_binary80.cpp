...
mplapack_binary80_t one_norm(mplapackint n, mplapack_binary80_t *a, mplapackint lda) { mplapack_binary80_t anorm=0.0; for(mplapackint j=0;j<n;j++){ mplapack_binary80_t s=0.0; for(mplapackint i=0;i<n;i++) s=s+abs(a[i+j*lda]); if(anorm<s) anorm=s; } return anorm; }
int main() {
    mplapackint n = 2, lda = n, info;
    mplapack_binary80_t *a = new mplapack_binary80_t[n*n]; mplapack_binary80_t *aorg = new mplapack_binary80_t[n*n]; mplapack_binary80_t *work = new mplapack_binary80_t[3*n]; mplapackint *iwork = new mplapackint[n]; mplapack_binary80_t rcond=0.0;
