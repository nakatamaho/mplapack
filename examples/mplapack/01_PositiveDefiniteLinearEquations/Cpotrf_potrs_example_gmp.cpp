...
int main() {
    ...
    mpc_class *a = new mpc_class[n*n]; mpc_class *aorg = new mpc_class[n*n]; mpc_class *b = new mpc_class[n*nrhs]; mpc_class *borg = new mpc_class[n*nrhs]; mpc_class *xexact = new mpc_class[n*nrhs];
    a[0]=mpc_class(5.0,0.0); a[1]=mpc_class(1.0,-1.0); a[2]=mpc_class(1.0,1.0); a[3]=mpc_class(4.0,0.0);
    xexact[0]=mpc_class(1.0,1.0); xexact[1]=mpc_class(2.0,-1.0); xexact[0+n]=mpc_class(-1.0,0.0); xexact[1+n]=mpc_class(0.0,2.0);
    for(mplapackint i=0;i<n*n;i++) aorg[i]=a[i];
    for(mplapackint j=0;j<nrhs;j++) for(mplapackint i=0;i<n;i++){ b[i+j*ldb]=mpc_class(0.0,0.0); for(mplapackint k=0;k<n;k++) b[i+j*ldb]=b[i+j*ldb]+aorg[i+k*lda]*xexact[k+j*n]; borg[i+j*ldb]=b[i+j*ldb]; }
