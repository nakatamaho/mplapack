...
void getAinv(mplapackint n, mplapackint lda, qd_real * A)
{
    mplapackint info;
    mplapackint lwork;
...
qd_real get_estimated_condition_num(const char *norm, mplapackint n, mplapackint lda, qd_real * A)
{
    qd_real anorm, cond, rcond;
    qd_real *work = new qd_real[std::max((mplapackint) 1, n * 4)];
...
qd_real get_exact_condition_num(const char *norm, mplapackint n, mplapackint lda, qd_real * A)
{
    qd_real *Ainv = new qd_real[n * n];
    qd_real *work = new qd_real[std::max((mplapackint) 1, n)];
    qd_real anorm, ainvnorm, cond;
//save A matrix
    for (int i = 0; i < n; i++) {
	for (int j = 0; j < n; j++) {
	    Ainv[i + j * n] = A[i + j * n];
