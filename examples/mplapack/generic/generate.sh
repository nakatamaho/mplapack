...
for _file in $FILES; do
    for _mplib in $MPLIBS; do
        ...
        if [ x"$_mplib" = x"gmp" ]; then
            sed -i -e "s/REAL/mpf_class/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi
        if [ x"$_mplib" = x"mpfr" ]; then
            sed -i -e "s/REAL/mpreal/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi
        if [ x"$_mplib" = x"double" ]; then
            sed -i -e "s/REAL/double/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi 
        if [ x"$_mplib" = x"dd" ]; then
            sed -i -e "s/REAL/dd_real/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi 
        if [ x"$_mplib" = x"qd" ]; then
            sed -i -e "s/REAL/qd_real/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi
        if [ x"$_mplib" = x"_Float128" ]; then
            sed -i -e "s/REAL/_Float128/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi
        if [ x"$_mplib" = x"_Float64x" ]; then
            sed -i -e "s/REAL/_Float64x/g" -e "s/INTEGER/mplapackint/g" ../$resultfilename
        fi
    done
done
