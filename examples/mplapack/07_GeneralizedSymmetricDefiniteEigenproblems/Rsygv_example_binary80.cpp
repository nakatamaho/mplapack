...
int main(){ mplapackint n=2,lda=n,ldb=n,info,lwork=-1; mplapack_binary80_t *a=new mplapack_binary80_t[n*n]; mplapack_binary80_t *b=new mplapack_binary80_t[n*n]; mplapack_binary80_t *aorg=new mplapack_binary80_t[n*n]; mplapack_binary80_t *borg=new mplapack_binary80_t[n*n]; mplapack_binary80_t *w=new mplapack_binary80_t[n]; for(mplapackint i=0;i<n*n;i++){a[i]=0;b[i]=0;} a[0]=2; a[3]=6; b[0]=1; b[3]=2; for(mplapackint i=0;i<n*n;i++){aorg[i]=a[i]; borg[i]=b[i];} mplapack_binary80_t wk; Rsygv((mplapackint)1,"V","U",n,a,lda,b,ldb,w,&wk,lwork,info); lwork=castINTEGER_binary80(wk); mplapack_binary80_t *work=new mplapack_binary80_t[lwork]; Rsygv((mplapackint)1,"V","U",n,a,lda,b,ldb,w,work,lwork,info); printf("eigenvalues = "); printvec(w,n); printf("\n"); printf("eigenvectors = "); printmat(n,n,a,lda); printf("\n"); for(mplapackint j=0;j<n;j++){ printf("residual[%ld] = ",(long)j); printnum(max_eigen_residual(n,aorg,borg,w[j],&a[j*lda])); printf("\n"); } delete[] work; delete[] w; delete[] borg; delete[] aorg; delete[] b; delete[] a; return info!=0?1:0; }
