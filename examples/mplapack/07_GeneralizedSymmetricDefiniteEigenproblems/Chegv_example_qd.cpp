...
int main(){ mplapackint n=2,lda=n,ldb=n,info,lwork=-1; qd_complex *a=new qd_complex[n*n]; qd_complex *b=new qd_complex[n*n]; qd_complex *aorg=new qd_complex[n*n]; qd_complex *borg=new qd_complex[n*n]; qd_real *w=new qd_real[n]; qd_real *rwork=new qd_real[3*n]; for(mplapackint i=0;i<n*n;i++){a[i]=qd_complex(0.0,0.0);b[i]=qd_complex(0.0,0.0);} a[0]=qd_complex(2.0,0.0); a[3]=qd_complex(6.0,0.0); b[0]=qd_complex(1.0,0.0); b[3]=qd_complex(2.0,0.0); for(mplapackint i=0;i<n*n;i++){aorg[i]=a[i]; borg[i]=b[i];} qd_complex wk; Chegv((mplapackint)1,"V","U",n,a,lda,b,ldb,w,&wk,lwork,rwork,info); lwork=castINTEGER_qd(wk.real()); qd_complex *work=new qd_complex[lwork]; Chegv((mplapackint)1,"V","U",n,a,lda,b,ldb,w,work,lwork,rwork,info); printf("eigenvalues = "); printvec(w,n); printf("\n"); printf("eigenvectors = "); printmat(n,n,a,lda); printf("\n"); for(mplapackint j=0;j<n;j++){ printf("residual[%ld] = ",(long)j); printnum(max_eigen_residual(n,aorg,borg,w[j],&a[j*lda])); printf("\n"); } delete[] work; delete[] rwork; delete[] w; delete[] borg; delete[] aorg; delete[] b; delete[] a; return info!=0?1:0; }
