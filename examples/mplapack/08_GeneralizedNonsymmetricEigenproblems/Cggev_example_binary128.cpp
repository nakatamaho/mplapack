...
int main(){ mplapackint n=3,lda=n,ldb=n,ldv=1,info,lwork=-1; std::complex<mplapack_binary128_t> *a=new std::complex<mplapack_binary128_t>[n*n]; std::complex<mplapack_binary128_t> *b=new std::complex<mplapack_binary128_t>[n*n]; std::complex<mplapack_binary128_t> *alpha=new std::complex<mplapack_binary128_t>[n]; std::complex<mplapack_binary128_t> *beta=new std::complex<mplapack_binary128_t>[n]; std::complex<mplapack_binary128_t> *vl=new std::complex<mplapack_binary128_t>[1]; std::complex<mplapack_binary128_t> *vr=new std::complex<mplapack_binary128_t>[1]; mplapack_binary128_t *rwork=new mplapack_binary128_t[8*n]; for(mplapackint i=0;i<n*n;i++){a[i]=std::complex<mplapack_binary128_t>(0.0,0.0);b[i]=std::complex<mplapack_binary128_t>(0.0,0.0);} a[0]=std::complex<mplapack_binary128_t>(1.0,1.0); a[4]=std::complex<mplapack_binary128_t>(2.0,0.0); a[8]=std::complex<mplapack_binary128_t>(3.0,-1.0); b[0]=std::complex<mplapack_binary128_t>(1.0,0.0); b[4]=std::complex<mplapack_binary128_t>(1.0,0.0); b[8]=std::complex<mplapack_binary128_t>(0.0,0.0); std::complex<mplapack_binary128_t> wk; Cggev("N","N",n,a,lda,b,ldb,alpha,beta,vl,ldv,vr,ldv,&wk,lwork,rwork,info); lwork=castINTEGER_binary128(wk.real()); std::complex<mplapack_binary128_t> *work=new std::complex<mplapack_binary128_t>[lwork]; Cggev("N","N",n,a,lda,b,ldb,alpha,beta,vl,ldv,vr,ldv,work,lwork,rwork,info); for(mplapackint i=0;i<n;i++){ printf("alpha[%ld] = ",(long)i); printnum(alpha[i]); printf(", beta = "); printnum(beta[i]); if(abs(beta[i])<=Rlamch_binary128("E")) printf(", lambda = Inf\n"); else { printf(", lambda = "); printnum(alpha[i]/beta[i]); printf("\n"); }} delete[] work; delete[] rwork; delete[] vr; delete[] vl; delete[] beta; delete[] alpha; delete[] b; delete[] a; return info!=0?1:0; }
