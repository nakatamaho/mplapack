...
//static mplapack_binary128_t *AB;
...
void init_gpu(int bs = 4)
{
  ...
  buffer_size_a_byte_global = 8*1024*1024*sizeof(mplapack_binary128_t);
  buffer_size_b_byte_global = 8*1024*1024*sizeof(mplapack_binary128_t);
  buffer_size_c_byte_global = 8*1024*1024*sizeof(mplapack_binary128_t);
  ...
  //  AB = (mplapack_binary128_t *)memalign(64, buffer_size_c_byte_global);
  //  assert(AB != NULL);
  __firstcall__ = 0;
		    ...
		    mplapack_binary128_t *C, int ldc)
{
  ...
  const size_t w_byte = sizeof(mplapack_binary128_t);
  ...
  result = clSetKernelArg(ker, 3, sizeof(mplapack_binary128_t), &alpha); assert(result == CL_SUCCESS);
  ...
  result = clSetKernelArg(ker, 8, sizeof(mplapack_binary128_t), &beta);  assert(result == CL_SUCCESS);
...
void Rgemm_ref(const char *transa, const char *transb,
	       ...
	       mplapack_binary128_t const alpha,
	       mplapack_binary128_t * A, mplapackint const lda,
	       mplapack_binary128_t * B, mplapackint const ldb,
	       mplapack_binary128_t const beta,
	       mplapack_binary128_t * C, mplapackint const ldc);
	   ...
	   mplapack_binary128_t * C, mplapackint const ldc)
{
  ...
  const mplapack_binary128_t Zero = 0.0, One = 1.0;
