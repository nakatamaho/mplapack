...
|---|---|---|---|---|
| **E1. BLAS temporaries inherit an unrelated gmpfrxx/MPFR default** | A routine declares `REAL temp = 0.0` or `REAL temp;`. For MPFR this constructs `temp` at the current thread default. Assignment evaluates a high-precision expression into that lower-precision destination before the value reaches C. | A 1024-bit `Rgemm` identity product loses `2^-700` when the caller leaves the default at 512 bits, but passes when the caller establishes 1024 bits. Left and right identity can differ because different GEMM branches place different operands in the temporary. | The caller selects one precision, constructs all participating objects at that precision, and establishes the current-thread default before entering MPLAPACK. Optimized OpenMP workers read one valid caller object's precision and enter a worker-local scope. MPLAPACK does not scan, promote, or check operands. | The uniform caller contract and OpenMP worker propagation are established for MPFR `Rgemm`; the same source pattern remains a risk in other BLAS/LAPACK routines until their worker contexts are audited. See `docs/mpfr-rgemm-operation-precision.md`. |
