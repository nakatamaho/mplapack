...
echo "/usr/bin/time wine ./*xeigtstR_double.exe < Rbal_double.in >& log.x86_64-w64-mingw32-xeigtstR_double.Rbal.in" >> .parallel.test_eig_all_mingw.sh
echo "/usr/bin/time wine ./*xeigtstC_double.exe < Cbal_double.in >& log.x86_64-w64-mingw32-xeigtstC_double.Cbal.in"  >> .parallel.test_eig_all_mingw.sh

for eigreal in $EIGREALS; do
    for testreal in $TESTREALS; do
        echo "/usr/bin/time wine ./$eigreal < $testreal >& log.$eigreal.$testreal" >> .parallel.test_eig_all_mingw.sh
    done
done

for eigcomplex in $EIGCOMPLEXES; do
    for testcomplex in $TESTCOMPLEXES; do
        echo "/usr/bin/time wine ./$eigcomplex < $testcomplex >& log.$eigcomplex.$testcomplex" >> .parallel.test_eig_all_mingw.sh
    done
done
