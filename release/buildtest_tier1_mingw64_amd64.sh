...
: "${MPLAPACK_REMOTE_WORKDIR:=${HOME}/tmp/mplapack-mingw64-amd64}"
: "${MPLAPACK_REF:=master}"
: "${MPLAPACK_DISTRO_VERSION:=24.04}"
: "${MPLAPACK_DOCKER_BASE:=ubuntu:${MPLAPACK_DISTRO_VERSION}}"
: "${MPLAPACK_DOCKERFILE:=release/docker/distcheck/Dockerfile.mingw}"
: "${MPLAPACK_DOCKER_CONTEXT:=release/docker}"
: "${MPLAPACK_IMAGE_TAG:=mplapack-tier1-mingw64-amd64:latest}"
...
if mkdir "${LOCKDIR}" 2>/dev/null; then
    ...
    if [ -n "${old_pid}" ] && [ "${old_pid}" != "$$" ] && kill -0 "${old_pid}" 2>/dev/null; then
        log "Another tier1-mingw64-amd64 buildtest is running (pid: ${old_pid}); stopping it."
