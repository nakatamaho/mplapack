...
`make dist` reuses `release/dist-cache/` when the Git source key matches. A clean tree is keyed by `HEAD`; a tracked dirty tree is keyed by `HEAD` plus the patch SHA. Set `MPLAPACK_DIST_CACHE_REUSE=no` to force regeneration, or `MPLAPACK_DIST_CACHE_DIR=/path/to/cache` to use a different cache directory.
