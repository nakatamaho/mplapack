...
# Logging setup
# ---------------------------------------------------------------------------
LOG_DIR="${HOME}/mplapack_build_logs/$(LANG=C date +%Y%m%d_%H%M%S)_$$_tier2_i386_debian"
mkdir -p "${LOG_DIR}"
...
run_step "reconfig"       bash misc/reconfig.ubuntu24.04.sh
...
run_step "make_distcheck" env CC="ccache gcc" CXX="ccache g++" FC="ccache gfortran" \
                          make distcheck MAKEFLAGS="-j32" DISTCHECK_CONFIGURE_FLAGS="${DISTCHECK_CONFIGURE_FLAGS}"
