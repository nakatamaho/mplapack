...
cat hand/*hpp *hpp ~/mplapack/misc/special.hpp | grep -v abs1 | grep -v abs2 | grep -v abssq | grep -v arr_c | grep -v common | grep -v UNHANDLED_function | grep -v arr_ref | grep -v  mplapack_Rlaruv_initialize | sort  |uniq > header_all
