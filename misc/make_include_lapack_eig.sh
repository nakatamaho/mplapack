...
cat hand/*hpp *hpp ~/mplapack/misc/special.hpp | grep -v abs1 | grep -v abs2 | grep -v abssq | grep -v arr_c | grep -v UNHANDLED_function | grep -v arr_ref | grep -v random_mplapack_gmp | grep -v mplapack_Rlaruv_gmp_initialize | grep -v mplapack_Rlaruv_mpfr_initialize | grep -v main | grep -v Rchkee | grep -v Cchkee | grep -v Cslect | grep -v Rslect | grep -v _Clctsx | sort |uniq > header_all
...
for mplib in $MPLIBS; do
    ...
    if [ x"$mplib" = x"_Float128" ]; then
        ...
        sed -i -e 's/REAL/_Float128/g' mplapack_eig_${mplib}.h 
        sed -i -e "s/Rlamch/Rlamch_${mplib}/g" mplapack_eig_${mplib}.h 
        sed -i -e "s/Mlsamen/Mlsamen_${mplib}/g" mplapack_eig_${mplib}.h
        sed -i -e "s/iMlaenv2stage/iMlaenv2stage_${mplib}/g" mplapack_eig_${mplib}.h 
        sed -i -e "s/iMlaenv(/iMlaenv_${mplib}(/g" mplapack_eig_${mplib}.h 
    ...
    if [ x"$mplib" = x"_Float64x" ]; then
        ...
        sed -i -e 's/REAL/_Float64x/g' mplapack_eig_${mplib}.h 
        sed -i -e "s/Rlamch/Rlamch_${mplib}/g" mplapack_eig_${mplib}.h 
        sed -i -e "s/Mlsamen/Mlsamen_${mplib}/g" mplapack_eig_${mplib}.h
        sed -i -e "s/iMlaenv2stage/iMlaenv2stage_${mplib}/g" mplapack_eig_${mplib}.h 
        sed -i -e "s/iMlaenv(/iMlaenv_${mplib}(/g" mplapack_eig_${mplib}.h 
    ...
    cat ~/mplapack/misc/mplapack_eig_${mplib}.h.in mplapack_eig_${mplib}.h > ~/mplapack/include/mplapack_eig_${mplib}.h
