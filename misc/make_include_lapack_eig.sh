...
cat hand/*hpp *hpp ~/mplapack/misc/special.hpp | grep -v abs1 | grep -v abs2 | grep -v abssq | grep -v arr_c | grep -v UNHANDLED_function | grep -v arr_ref | grep -v random_mplapack_gmp | grep -v mplapack_Rlaruv_gmp_initialize | grep -v mplapack_Rlaruv_mpfr_initialize | grep -v main | grep -v Rchkee | grep -v Cchkee | sort |uniq > header_all
