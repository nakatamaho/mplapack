...
fable.cout $1 | sed -e 's/max(1/max((INTEGER)1/g'  -e 's/fem::double0/0\.0/g' | sed 's/fem::bool0/false/g' | sed 's/fem::int0/0/g' | sed 's/fem::fint/int/g' | sed 's/int/INTEGER/g' | sed -e "/dimension/d" | sed 's/fem:://g' | sed 's/double/REAL/g' | sed 's|//C|//|g' | sed -e 's/str_cref/const char */g' -e 's/arr_cref<REAL>/REAL \*/g' -e 's/arr_ref<REAL>/REAL \*/g'| sed -e 's/arr_cref<INTEGER>/INTEGER \*/g' | sed -e 's/arr_ref<REAL, 2> /REAL \*/g' -e 's/0\.0e0/0\.0/g' -e 's/1\.0e0/1\.0/g' -e 's/0\.0e+0/0\.0/g' -e 's/1\.0e+0/1\.0/g' -e 's/dabs/abs/g' -e 's/arr_cref<REAL, 2> /REAL \*/g' -e 's/std::complex<REAL>/COMPLEX/g' -e 's/_MPLAPACK_REPLACE_dble//g' -e 's/_MPLAPACK_REPLACE_dimag//g' -e 's/_MPLAPACK_REPLACE_dsign/sign/g' -e 's/_MPLAPACK_REPLACE_dconjg/conj/g' -e 's/_MPLAPACK_REPLACE_dsqrt/sqrt/g' -e 's/1\.e0/1\.0/g' -e 's/2\.e0/2\.0/g' -e 's/4096\.e0/4096\.0/g' -e 's/16777216\.e0/16777216/g' -e 's/0\.e0/0\.0/g' -e 's/\.0e0/.0/g' -e 's/arr_cref<COMPLEX, 2> /COMPLEX \*/g' -e 's/COMPLEX0/ \(0\.0, 0\.0\)/g'  > ${output}_
