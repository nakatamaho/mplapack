...
#endif
#if defined ___MPLAPACK_BUILD_WITH_BINARY128___
    mpreal(const binary128 &a, mp_prec_t prec = default_prec, mp_rnd_t mode = default_rnd);
    mpreal &operator=(const binary128 &a);
#endif
};
...
#if defined ___MPLAPACK_BUILD_WITH_BINARY128___
inline mpreal &mpreal::operator=(const binary128 &a) {
    mpfr_init2((mpfr_ptr)mp, default_prec);
    mpfr_set_float128((mpfr_ptr)mp, a, default_rnd);
    return *this;
}

inline mpreal::mpreal(const binary128 &a, mp_prec_t prec, mp_rnd_t mode) {
    mpfr_init2(mp, prec);
    mpfr_set_float128(mp, a, mode);
}

inline const mpreal operator-(const binary128 a, const mpreal b) { return mpreal(a) -= b; }

inline const mpreal operator-(const mpreal &a, const binary128 &b) {
    mpreal tmp(b);
    return -(mpreal(b) -= a);
}

inline binary128 cast2binary128(const mpreal &b) {
    // mpreal -> mpfr -> binary128
    binary128 q;
    mpreal a(b);
